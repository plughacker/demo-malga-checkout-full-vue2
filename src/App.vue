<template>
  <malga-checkout-full
    ...
    :pageConfig.prop="{
      footerDescription:
        'Todos os direitos reservados © 2022 Malga.',
...
import {
  ...
  MalgaCheckoutFullVueData,
} from "./App.types";

export default {
  data(): MalgaCheckoutFullVueData {
    return {
  ...
  methods: {
    handlePaymentSuccess(data: MalgaCheckoutFullTransactionSuccessEvent): void {
      console.log(data);
    },
    handlePaymentFailed(error: MalgaCheckoutFullTransactionErrorEvent): void {
      console.log(error);
    },
  },
};
